...
// Encrypt stores sensitive material.
...
func Encrypt(secretsKey, plaintext string) (string, error) {
	if plaintext == "" {
		return "", nil
	}
	if secretsKey == "" {
...
func Decrypt(secretsKey, stored string) (string, error) {
	if stored == "" {
		return "", nil
	}
	...
	if secretsKey == "" {
		...
		return "", fmt.Errorf("secrets key required to decrypt")
	}
