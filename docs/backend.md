...
The WebSocket is in `ws.go`. Refer to `api.md` for the WebSocket details.
...
The migrations are applied in `migrate.go`. Refer to `database.md` for the schema.
...
Refer to `api.md` for the API contract and to `development.md` for the build commands.