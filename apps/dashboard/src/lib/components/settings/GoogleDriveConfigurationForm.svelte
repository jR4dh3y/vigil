<script lang="ts">
	...
	async function handleSubmit(event: SubmitEvent) {
		...
		const parsed = configurationSchema.safeParse({ clientId, clientSecret, redirectUrl });
		if (!parsed.success) {
...
<form class="mt-5 flex flex-col gap-4" onsubmit={handleSubmit} novalidate>
	<div class="flex flex-col gap-1.5">
		<label for="gdrive-client-id" class="text-sm font-medium text-zinc-300">Client ID</label>
		<input
			id="gdrive-client-id"
			name="clientId"
			type="text"
	...
	<div class="flex flex-col gap-1.5">
		<label for="gdrive-client-secret" class="text-sm font-medium text-zinc-300">Client secret</label>
		<input
	...
	<div class="flex flex-col gap-1.5">
		<label for="gdrive-redirect-url" class="text-sm font-medium text-zinc-300">Redirect URL</label>
		<input
